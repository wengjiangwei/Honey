...
# ROOT = Path(os.path.relpath(FILE.parents[0] , Path.cwd()))  # relative
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    #filter some results
                    if (filter_img is not None) and func_filter_img(xyxy,names[int(cls)],filter_img,im0s):
                        continue
                    objects={}
                ...
                if save_xml == True:
                    try:
                        w = im0s.shape[0]
                        h = im0s.shape[1]
                        plots.write_xml(path, results, w, h, save_dir)
                        plots.write_image(path, im0s, save_dir)
                    except:
                        continue
